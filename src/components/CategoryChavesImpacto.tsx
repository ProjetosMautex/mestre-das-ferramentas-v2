...
export const CategoryChavesImpacto: React.FC = () => {
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 9;
  const [inputPage, setInputPage] = useState('');

  const filteredArticles = articlesChaveDeImpacto;
    ...
    <main className="flex-grow bg-gray-50">
      <section className="bg-[#1a1a1a] py-16 text-center">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h1 className="text-4xl font-bold text-white mb-4">Chaves de Impacto</h1>
          <p className="text-gray-400 max-w-2xl mx-auto text-lg">
            As melhores ferramentas para serviços pesados e alto torque.
          </p>
        </div>
      </section>
      ...
      <section className="py-16 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 bg-white">
        ...
        {filteredArticles.length > 0 ? (
          ...
          <div className="py-20 bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200 text-center">
            <h2 className="text-3xl font-bold text-gray-400 mb-4">Em Breve!</h2>
            <p className="text-gray-500 max-w-md mx-auto text-lg">
              Estamos preparando análises técnicas e comparativos das melhores chaves de impacto do mercado. Fique atento às atualizações!
            </p>
          </div>
        )}
